...
let priority_unrop = function NOT -> 1 | UMINUS -> 7
...
let pp_expression =
    let rec ppl pr = function
        ...
        | ExpStr s -> "\"" ^ s ^ "\""
        | ExpUnr (op, e) ->
            let res = (pp_unrop op) ^ (ppl (priority_unrop op) e)
            in if pr=0 then res else paranthesis res
        | ExpBin (e1, op, e2) ->
            let pr2 = priority_binop op
            in let res = (ppl pr2 e1) ^ (pp_binop op) ^ (ppr pr2 e2)
            in if pr2 >= pr then res else paranthesis res
...
let forward_n cl = cl.current <- cl.current + n
let extract pred cl =
...
let reduce pr = function
    (Texp e) :: (Tunr op) :: st when (priority_unrop op) >= pr ->
        (Texp (ExpUnr (op, e))) :: st
